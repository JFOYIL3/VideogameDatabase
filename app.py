...
def avg_year():
    ...
    if (year_average - int(year_average) >= 0.5):
        year_average = int(year_average) + 1
    elif (year_average - int(year_average) < 0.5):
        year_average = int(year_average)
    print("\nThe average release year of your games is " + str(year_average) + ".")


def list_games():
    for row in crsr.execute("""SELECT * FROM videogames"""):
        print("{:60}  {:20}  {:40}  {}".format(row[0], row[1], row[2], row[3]))
        ...
        if choice == "1":
            ...
            if (year,) in rows:
                ...
                for row in crsr.execute("""SELECT * FROM videogames WHERE year_released = "{}" """.format(year)):
                    print("{:60}  {:20}  {:40}  {}".format(row[0], row[1], row[2], row[3]))
                    game_count += 1
                print("\nYou have " + str(game_count) + " games released in " + str(year) + ".")
            else:
